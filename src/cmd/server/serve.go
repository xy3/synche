...
import (
	...
	"github.com/xy3/synche/src/ftp"
	server2 "github.com/xy3/synche/src/server"
	"github.com/xy3/synche/src/server/restapi"
...
func startHttpServer(flags *flag.FlagSet, sig chan struct{}) error {
	...
	apiServer.Host = server2.Config.Server.Host
...
func init() {
	...
	serveCmd.Flags().IntVar(&server2.Config.Ftp.Port, "ftp-port", 2121, "port for the ftp server to run on")
	serveCmd.Flags().StringVar(&server2.Config.Ftp.Hostname, "ftp-hostname", "127.0.0.1", "ftp service listen IP")
	serveCmd.Flags().StringVar(&server2.Config.Ftp.PublicIp, "ftp-public-ip", "", "ftp client connect to ftp server to transfer data in passive mode")
	serveCmd.Flags().StringVar(&server2.Config.Ftp.PassivePorts, "ftp-passive-port-range", "52013-52114", "ftp server will pick a port random in the range, open the data connection tunnel in passive mode")
	serveCmd.Flags().StringVar(&server2.Config.Ftp.WelcomeMessage, "ftp-welcome-message", "Welcome to the Synche FTP server", "ftp server welcome message")
