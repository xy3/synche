...
func createDirByPath() {
	params := files2.NewCreateDirPathParams().WithDirPath(newDirPath).WithContext(context.Background())
	directory, err := client.Client.Files.CreateDirPath(params, client.ClientAuth)
...
func createDirByID(name string, parentID uint64) {
	...
	if parentID != 0 {
		...
		directory, err := client.Client.Files.CreateDirectory(&files2.CreateDirectoryParams{
			DirectoryName:     name,
