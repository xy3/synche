...
func ListDirByPath() *models.DirectoryContents {
	params := files2.NewListDirPathInfoParams().WithDirPath(listDirPath)

	resp, err := client.Client.Files.ListDirPathInfo(params, client.ClientAuth)
	if err != nil {
...
func ListDirectoryByID() *models.DirectoryContents {
	params := files2.NewListDirectoryParams().WithID(listDirID)

	resp, err := client.Client.Files.ListDirectory(params, client.ClientAuth)
	if err != nil {
		log.WithError(err).Fatal("Failed to retrieve directory contents")
	}
...
func ListHomeDirectory() *models.DirectoryContents {
	resp, err := client.Client.Files.ListHomeDirectory(files2.NewListHomeDirectoryParams(), client.ClientAuth)
	if err != nil {
		log.WithError(err).Fatal("Failed to retrieve directory contents")
	}
