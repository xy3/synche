...
func deleteUserJob() error {
	...
	if input == "y" || input == "yes" {
		_, err := client.Client.Users.DeleteUser(users.NewDeleteUserParams().WithEmail(deleteUserEmail), client.ClientAuth)
		if err != nil {
			return err
		}
...
func deleteJobByPath() error {
	_, err := client.Client.Files.DeleteFilepath(
		files2.NewDeleteFilepathParams().WithFilePath(
			deleteFilepath).WithContext(
			context.Background()), client.ClientAuth)
	if err != nil {
		return err
	}
...
func deleteJobByID() error {
	_, err := client.Client.Files.DeleteFile(
		files2.NewDeleteFileParams().WithFileID(
