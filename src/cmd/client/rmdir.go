...
func removeDirByPath() (deleted bool, err error) {
	...
	params := files2.NewDeleteDirPathParams().WithDirPath(rmDirPath).WithContext(context.Background())
	_, err = client.Client.Files.DeleteDirPath(params, client.ClientAuth)
...
func removeDirByID() (deleted bool, err error) {
	...
	_, err = client.Client.Files.DeleteDirectory(&files2.DeleteDirectoryParams{
		ID:      rmDirID,
